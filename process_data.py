...
def process_keystroke_data():
    ...
    for _, row in gt_cs1.iterrows():
        ...
        if pd.notna(row["file_1"]):
            ...
            if file_path.exists():
                process_single_file(
                    file_path, pID, has_parkinsons, updrs, typing_speed, all_keystrokes
                )
...
def process_single_file(
    ...
    try:
        # Read the CSV file
        df = pd.read_csv(
            file_path, header=None, names=["key", "press_0", "release_0", "duration"]
        )
...
def remove_outliers(df):
    ...
    df_cleaned = df_cleaned[
        df_cleaned["duration"] < 0.5
    ]  # Remove durations > 0.5 seconds
