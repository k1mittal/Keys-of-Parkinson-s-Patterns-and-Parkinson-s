...
class NqDataLoader:
    ...
    def sanityCheck(self):
        ...
        nonConsTmpLbl = (
            np.zeros(len(self.dataTimeStart)) == 0
    ...
    def loadDataFile(self, fileIn, autoFilt=True, impType=None, debug=False):
        ...
        try:
            ...
            if impType == "si":
                data = np.genfromtxt(
                    fileIn, dtype=int, delimiter=",", skip_header=0, encoding="utf-8"
                )
                data = data - data.min()
                ...
                data = np.genfromtxt(
                    fileIn, dtype=None, delimiter=",", skip_header=0, encoding="utf-8"
                )
                # load
                self.dataKeys = data["f0"]

                if self.dataKeys.dtype.kind == "S":
                    # this returns an array of dtype '<U…' (unicode)
                    self.dataKeys = np.char.decode(self.dataKeys, "utf-8")
            ...
            self.dataFT = np.array(
                [
                    ...
                    for i in range(1, self.dataTimeStart.size)
                ]
            )
    ...
    def filtData(self, flags):
        # ensure dataKeys is Unicode, not bytes
        if hasattr(self.dataKeys, "dtype") and self.dataKeys.dtype.kind == "S":
            # This turns an array of b'…' into an array of '…'
            self.dataKeys = self.dataKeys.astype("U")
        """
        Filter data
        ...
        pShortMeta = re.compile(
            '("space")|("Num_Lock")|("Return")|("P_Enter")|("Caps_Lock")|("Left")|("Right")|("Up")|("Down")'
        )
        pPunct = re.compile(
            '("more")|("less")|("exclamdown")|("comma")|("\[65027\]")|("\[65105\]")|("ntilde")|("minus")|("equal")|("bracketleft")|("bracketright")|("semicolon")|("backslash")|("apostrophe")|("comma")|("period")|("slash")|("grave")'
        )
        # --
